...
def equations(concs, t, *ks):
    ...
    return [ - k1*Ac*E + k_2*An + 2*(kw+k4*Ac)*An - (k1*Ac**2*E)/(Ac+K) - (k_2*An*Ac)/(Ac+K)  
                + (k1*K*Ac*E)/(Ac+K) + (k_2*K*An)/(Ac+K),
             - k1*Ac*E,
             + k1*Ac*E,
             - k_2*An + (k1*Ac**2*E)/(Ac+K) + (k_2*An*Ac)/(Ac+K) - (kw+k4*Ac)*An ]
...
model = KineticModel(
    ...
    parameter_names = ["k1", "k-2", "k4", "kw", "K", "[Acid]0", "[EDC]0", "[U]0", "[An]0"],
